...
webpage = requests.get("http://catalog.citruscollege.edu/programs-study/graduation-requirements-associate-degree/general-education-requirements-transfer-csu-csuge-option-ii/#csugerequirementstext")
    ...
    if ":" in table_data[i].get_text() or "," in table_data[i].get_text():
        if '.' in table_data[i+1].get_text():
            ...
            conn.execute("CREATE TABLE {} \
                         ...
                         course_units TEXT NOT NULL);".format(current_table))
            i += 2
        else:
        ...
        sql = "INSERT INTO {} (course_name, course_description, course_units) \
                VALUES (?, ?, ?)".format(current_table)
        conn.execute(sql, (course_name, course_description, course_units))
        i += 3
